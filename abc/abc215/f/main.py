...
min_x = SegTree(op=min, e = lambda : 10**9, n = n)
min_y = SegTree(op=min, e = lambda : 10**9, n = n)
max_x = 0
max_y = 0
ans = 0
for i in range(n):
    ...
    x,y = xy[a]
    idx = encode[a]
    ans = max(ans, max_x-x, max_y-y, x-min_x.prod(idx,n), y-min_y.prod(0,idx))
    max_x = max(max_x, x)
    max_y = max(max_y, y)
    min_x.set(idx, x)
    min_y.set(idx, y)
...
めんどくさいけど、(x-y)の値で座圧して
4つセグ木があればなんとかなる

→　最大のxと最大のｙはセグ木なしで管理できる
