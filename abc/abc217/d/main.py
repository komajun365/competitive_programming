...
l,q,*cx = map(int,read().split())

xs = list(set(cx[1::2])) + [0,l]
xs.sort()
...
st_max = SegTree(op=max, e = lambda :0 , n=n)
st_min = SegTree(op=min, e = lambda :l , n=n)

ans = []
it = iter(cx)
for c,x in zip(it,it):
    if c == 1:
        st_max.set(encode[x], x)
        st_min.set(encode[x], x)
    else:
        length = st_min.prod(encode[x],n) - st_max.prod(0,encode[x])
        ans.append(length)
