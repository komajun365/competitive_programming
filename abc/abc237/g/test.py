...
for _ in range(1000):
    ...
    clr = []
    for __ in range(q):
        clr.append(random.randint(1,2))
        a,b = rand_pair(n)
        clr += [a+1, b+1]
    ans1 = calc(n,q,x,p,clr)
    ans2 = calc_simple(n,q,x,p,clr)
    if ans1 != ans2:
        print(n,q,x,p,clr)
        print(ans1,ans2)
        exit()
