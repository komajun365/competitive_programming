...
mf.add_edge(0,1,inf)
mf.add_edge(n*2-2,n*2-1,inf)

it = iter(ab)
for ai,bi in zip(it,it):
    ai -= 1
    bi -= 1
    mf.add_edge(ai*2+1,bi*2,inf)
    mf.add_edge(bi*2+1,ai*2,inf)

print(mf.flow(0,n*2-1))
s = set()
t = set()
for i,can_go in enumerate(mf.min_cut(0)):
    if can_go:
        s.add(i)
    else:
        t.add(i)
...
block = []
for i in range(n):
    if i*2 in s and i*2+1 in t:
        block.append(i+1)
print(len(block))
print(*block)

# block = []
# for from_, to, cap, flow in mf.edges():
#     # print(from_, to, cap, flow)
#     if from_ + 1 == to and cap == flow:
#         block.append(from_ // 2 + 1)
# print(len(block))
# print(*block)
