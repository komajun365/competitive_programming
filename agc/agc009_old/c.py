...
for i,si in enumerate(s[2:],1):
    a_left = bisect.bisect_right(s,si-a)
    a_new = lst_b.prod(0,a_left)
    b_left = bisect.bisect_right(s,si-b)
    b_new = lst_a.prod(0,b_left)

    lst_a.set(i,a_new)
    lst_b.set(i,b_new)

    if(si - s[i] < a):
        lst_a.apply_lr(0,i,0)
    if(si - s[i] < b):
        lst_b.apply_lr(0,i,0)

    # lst_a.print()
    # lst_b.print()
    # print('===================')

ans = lst_a.all_prod() + lst_b.all_prod()
ans %= mod
print(ans)
