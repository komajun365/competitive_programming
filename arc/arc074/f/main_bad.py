...
t_num = t[0] * w + t[1]
for j in cols[s[0]]:
    mf.add_edge(s_num, s[0]*w + j, inf)
for i in rows[s[1]]:
    mf.add_edge(s_num, i*w + s[1], inf)

for j in cols[t[0]]:
    mf.add_edge(t[0]*w + j, t_num, inf)
for i in rows[t[1]]:
    mf.add_edge(i*w + t[1], t_num, inf)

# for i in range(h):
#     for j in cols[i]:
#         mf.add_edge(n+i*w+j, 2*n+i, 1)
#         mf.add_edge(2*n+i,i*w+j, 1)

# for j in range(w):
#     for i in cols[j]:
#         mf.add_edge(n+i*w+j, 2*n+h+j, 1)
#         mf.add_edge(2*n+h+j,i*w+j, 1)
...
for i in range(h):
    for j in range(w):
        for k in cols[i]:
            if j == k:
                continue
            mf.add_edge(n+i*w+j, i*w+k, inf)
        for k in rows[j]:
            if i == k:
                continue
            mf.add_edge(n+i*w+j, k*w+j, inf)


ans = mf.flow(s_num,t_num)
print(ans)
