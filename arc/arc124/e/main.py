...
# dp[j][0] := i+1番目の人がi番目からもらったボールを選ぶ
# dp[j][1] := i+1番目の人がi番目からもらったボールを選ばない
# dp[1] := 1回でもボールを渡さなかった人が出現した
def calc(dp):
    for i in range(n):
        dp2 = [[0,0],[0,0]]
        for j in range(2):
            dp2[j][0] += dp[j][0] * com(a[i]+1, 2)
            dp2[j][0] += dp[j][1] * com(a[i]+1, 3)
            dp2[j][0] %= mod
            dp2[j][1] += dp[j][0] * com(a[i]+1, 1)
            dp2[j][1] += dp[j][1] * com(a[i]+1, 2)
            dp2[j][1] %= mod
        
        move0 = dp[0][0]
        move1 = dp[0][1] * a[i] % mod
        dp2[0][1] -= move0 + move1
        dp2[0][1] %= mod
        dp2[1][1] += move0 + move1
        dp2[1][1] %= mod
