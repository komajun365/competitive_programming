...
mask = (1 << 32) - 1
# D,I,S,C,O,DI,IS,SC,CO,DIS,ISC,SCO,DISC,ISCO,DISCO
def encode(disco):
    res = 0
    for i in disco:
        res = (res << 32) + i
...
def op(x,y):
    ...
    res = [0] * 15
    for i in range(5):
        res[i] = (dx[i] + dy[i]) & mask
    for i in range(5,9):
        res[i] = (dx[i] + dy[i] + dx[i-5] * dy[i-4]) & mask
    for i in range(9,12):
        res[i] = (dx[i] + dy[i] + dx[i-9] * dy[i-3] + dx[i-4] * dy[i-7]) & mask
    for i in range(12,14):
        res[i] = (dx[i] + dy[i] + dx[i-12] * dy[i-2] 
                                + dx[i-7] * dy[i-5]
                                + dx[i-3] * dy[i-9]) & mask
    res[14] = (dx[14] + dy[14] + dx[0] * dy[13] 
                               + dx[5] * dy[11]
                               + dx[9] * dy[8]
                               + dx[12] * dy[4]) & mask
    return encode(res)
...
# di = 'D,I,S,C,O,DI,IS,SC,CO,DIS,ISC,SCO,DISC,ISCO,DISCO'.split(',')
# for i in range(5):
#     print(di[i])
# for i in range(5,9):
#     print(di[i])
