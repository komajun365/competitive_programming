...
seg_a = SegTree(op=op, e=e, v=va)
seg_b = SegTree(op=op, e=e, v=vb)
a = [0] * n
b = [0] * m
...
for t,x,y in zip(it,it,it):
    ...
    if t == 1:
        bef = a[x]
        left = seg_b.prod(0,encode[bef])
        right = seg_b.prod(0,encode[y])
        l_sum,l_cnt = divmod(left,base)
        r_sum,r_cnt = divmod(right,base)
        now += -(r_sum-l_sum) + y * r_cnt - bef * l_cnt
        ans.append(now)

        a[x] = y
        seg_a.add(encode[bef], -1 * ((bef<<20) + 1))
        seg_a.add(encode[y], ((y<<20) + 1))
    
    else:
        bef = b[x]
        left = seg_a.prod(0,encode[bef])
        right = seg_a.prod(0,encode[y])
        l_sum,l_cnt = divmod(left,base)
        r_sum,r_cnt = divmod(right,base)
        now += -(r_sum-l_sum) + y * r_cnt - bef * l_cnt
        ans.append(now)
        ...
        b[x] = y
        seg_b.add(encode[bef], -1 * ((bef<<20) + 1))
        seg_b.add(encode[y], ((y<<20) + 1))
