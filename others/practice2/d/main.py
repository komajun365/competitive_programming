...
for i in range(n):
    for j in range(m):
        ...
        if(s[i+1][j+1] == '#'):
            continue
        if((i+j)%2):
            mf.add_edge(v-2,i*m+j,1)
            for k,l in zip([-1,0,0,1],[0,-1,1,0]):
                k += i
                l += j
                if(s[k+1][l+1] == '#'):
                    continue
                mf.add_edge(i*m+j, k*m+l, 1)
        else:
            mf.add_edge(i*m+j,v-1,1)

print(mf.flow(v-2, v-1))
for i,j,cap,flow in mf.edges():
    if(i==v-2) or (j==v-1) or (flow==0):
        continue
    if(i-j == m):
        ans[i] = '^'
        ans[j] = 'v'
