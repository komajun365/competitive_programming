...
# w,n,*lrv = map(int,read().split())

# inf = -1 * 10**15
# st = SegTree(op=max, e=lambda : inf, n=(w+1))
# st.set(0,0)
...
#         left = max(0, j - r)
#         right = j - l + 1
#         st.set(j, max(st.get(j), st.prod(left, right) + v))
...
for l,r,v in zip(it,it,it):
    ...
    left = w
    for j in range(w,l-1,-1):
        while left >= 0 and left >= j-r:
            while dq:
                if dq[-1][0] <= dp[left]:
                    dq.pop()
                else:
                    break
            dq.append((dp[left],left))
            left -= 1
        while dq[0][1] > j-l:
            dq.popleft()
        dp[j] = max(dp[j], dq[0][0] + v)
