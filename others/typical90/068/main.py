...
n,q,*txyv = map(int,read().split())

even = n//2
odd = (n-1)//2
inf = -1 * 10**15

st_even = SegTree(op = lambda a,b:a+b, e = lambda: 0, v=[inf]*even)
st_odd = SegTree(op = lambda a,b:a+b , e = lambda: 0, v=[inf]*odd)
...
def calc(x,y,v):
    ...
    lo = l//2
    ro = r//2
    sum_e = st_even.prod(le,re)
    sum_o = st_odd.prod(lo,ro)
    cnt_e = (re-le) * 2
    cnt_o = (ro-lo) * 2
    if le*2 == x+1 or re * 2 == x:
        sum_e += v
        cnt_e += 1
    else:
        sum_o += v
        cnt_o += 1
    if sum_e < 0 or sum_o < 0:
        return('Ambiguous')
    if cnt_e > cnt_o:
        return str(sum_e - sum_o)
    else:
        return str(sum_o - sum_e)


ans = []
it = iter(txyv)
for t,x,y,v in zip(it,it,it,it):
    x -= 1
    y -= 1
    if t == 0:
        if x % 2 == 0:
        ...
        ans.append(calc(x,y,v))
print('\n'.join(ans))


'''
(3,9) -> 45,67,89 34,56,78
2,5 1,4

(2,8) -> 23,45,67 34,56,78
1,4 1,4
