...
for i in range(n):
    x,y = axy[i*2:i*2+2]
    for dx,dy in zip([T,T,0,-T,-T,-T,0,T],[0,T,T,T,0,-T,-T,-T]):
        dx += x
        dy += y
        if 0 <= dx <= 10**9 and 0 <= dy <= 10**9:
            num = (dx<<30) + dy
            if num in b:
                mf.add_edge(i, b[num], 1)
...
if cnt == n:
    ...
    ans = []
    edges = mf.edges()
    for from_, to, cap, flow in edges:
        if flow == 0 or from_ >= n or to < n or to >= 2*n:
            continue
        # print(from_,to,cap,flow)
        to -= n
        x0,y0 = axy[from_*2:from_*2+2]
        x1,y1 = bxy[to*2:to*2+2]
        dx = x1-x0
        dy = y1-y0
        dir = 0
        if dx == -T:
